  ...
  const actualizarEstadoCardWoo = (estado: "activo" | "manual") => {
    setIntegraciones((prev) => {
      ...
      const next = base.map((item: any) => {
        const nombre = String(item?.nombre ?? item?.canal ?? "").toLowerCase();
        if (!nombre.includes("woo")) return item;

        encontrado = true;
        return {
          ...item,
          nombre: item?.nombre ?? "WooCommerce",
          estado,
  ...
  return (
    <>
      ...
      {modalWoo && (
        <WooCommerceModal
          isOpen={modalWoo}
          onClose={() => setModalWoo(false)}
          empresaId={usuario?.empresa_id || ''}
          onConectado={() => {
            setModalWoo(false);
            actualizarEstadoCardWoo("activo");
          }}
          onDesconectado={() => {
