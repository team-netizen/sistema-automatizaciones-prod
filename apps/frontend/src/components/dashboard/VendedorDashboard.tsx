  ...
  return (
    <div style={{ background: T.bg, color: T.text, display: 'flex', fontFamily: "'DM Sans', 'Nunito', sans-serif", height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', flex: 1, flexDirection: 'column', height: '100vh', minWidth: 0, overflow: 'hidden' }}>
        <header style={{ alignItems: 'center', background: T.surface, borderBottom: `1px solid ${T.border}`, boxShadow: '0 2px 12px rgba(0,0,0,0.04)', display: 'grid', gridTemplateColumns: '1fr auto 1fr', height: 60, padding: '0 24px', position: 'sticky', top: 0, zIndex: 20 }}>
          <div style={{ color: T.text, fontSize: 15, fontWeight: 700 }}>SIS AUTO / Panel de vendedor</div>
          <div style={{ alignItems: 'center', background: T.accentLight, borderRadius: 999, color: T.accent, display: 'inline-flex', fontSize: 12, fontWeight: 700, gap: 6, padding: '8px 12px' }}><span>📍</span><span>{usuario?.sucursal_nombre || sucursalId}</span></div>
          <div style={{ alignItems: 'center', display: 'flex', gap: 10, justifyContent: 'flex-end' }}><div style={{ background: T.danger, borderRadius: 999, color: '#fff', fontSize: 10, fontWeight: 700, minWidth: 18, padding: '3px 6px', textAlign: 'center' }}>{resumen?.alertasPendientes ?? 0}</div><div style={{ alignItems: 'center', background: T.accentLight, borderRadius: 999, color: T.accent, display: 'flex', fontSize: 13, fontWeight: 800, height: 36, justifyContent: 'center', width: 36 }}>{String(usuarioNombre).slice(0, 1).toUpperCase()}</div></div>
        </header>
