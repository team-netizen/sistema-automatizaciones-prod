...
type DashboardMetricsResponse = {
  metricas?: Partial<DashboardMetricas>;
};
...
function normalizeProductoCritico(item: unknown): ProductoCritico | null {
  ...
  if (typeof sku !== 'string' || typeof nombre !== 'string') return null;

  return {
    sku,
    nombre,
    stock_web: toNumber(row.stock_web),
    stock_wsp: toNumber(row.stock_wsp),
...
function normalizeProductosCriticos(data: unknown): ProductoCritico[] {
  if (!Array.isArray(data)) return [];
  return data
    .map((item) => normalizeProductoCritico(item))
    .filter((item): item is ProductoCritico => item !== null);
}

async function getProductosCriticosSafe(): Promise<ProductoCritico[]> {
  const service = operacionesService as typeof operacionesService & {
    getProductosCriticos?: () => Promise<unknown>;
  };

  if (typeof service.getProductosCriticos !== 'function') {
    return [];
  }

  const response = await service.getProductosCriticos();
  return normalizeProductosCriticos(response);
}
...
export const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        const [metricasResponse, productosCriticos] = await Promise.all([
          operacionesService.getDashboardMetrics() as Promise<DashboardMetricsResponse>,
          getProductosCriticosSafe(),
        ]);

        const metricasRaw = metricasResponse?.metricas ?? {};
        const totalCriticos =
          toNumber(metricasRaw.total_productos_criticos) || productosCriticos.length;

        setStats({
          metricas: {
            total_productos: toNumber(metricasRaw.total_productos),
            pedidos_mes: toNumber(metricasRaw.pedidos_mes),
            movimientos_recientes: toNumber(metricasRaw.movimientos_recientes),
            alertas_activas: toNumber(metricasRaw.alertas_activas),
            total_productos_criticos: totalCriticos,
          },
    ...
    <div className="space-y-10 animate-fadeIn w-full">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <MetricCard
          ...
          icon={
            <svg className="w-5 h-5 text-[#8B7AF0]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-5 bg-[#111C19] border border-[#1F2D29] rounded-[32px] p-8 shadow-xl shadow-black/20">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              { label: 'Sincronizacion', val: 'Hace 5min', color: 'text-white' },
              { label: 'Canales', val: '3 Activos', color: 'text-white' },
              { label: 'Rotacion', val: '3.2x Mensual', color: 'text-white' },
            ].map((item) => (
              <div key={item.label} className="bg-[#0B1412] border border-[#1F2D29] rounded-2xl p-5 hover:bg-[#111C19] transition-all cursor-default">
        ...
        <div className="lg:col-span-7 bg-[#111C19] border border-[#1F2D29] rounded-[32px] p-8 shadow-xl shadow-black/20 flex flex-col md:flex-row items-center gap-10 relative overflow-hidden">
          ...
          <div className="flex-1 space-y-5 z-10">
            <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Trafico por Canal</h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                <div key={canal.name} className="flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${canal.color} shadow-[0_0_8px_rgba(255,255,255,0.1)]`}></span>
                    <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest">{canal.name}</span>
                  </div>
                  <span className="text-xl font-black text-white">{canal.perc}%</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="w-full min-h-[500px]">
        <OperationTable
          title="Resumen de Inventario Critico"
          columns={columns}
          data={stats?.productos_criticos || []}
          isLoading={loading}
          totalEntries={stats?.metricas?.total_productos_criticos || 0}
          accentColor="#8B7AF0"
          emptyMessage="No hay stock critico detectado"
          renderRow={(item) => {
              ...
              <tr key={item.sku} className="group hover:bg-[#0B1412]/40 transition-all cursor-pointer">
                ...
                <td className="px-6 md:px-8 py-5 border-b border-[#1F2D29]/30">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-gray-600 font-bold uppercase tracking-widest">Almacen Central</span>
                  </div>
                </td>
                ...
                <td className="px-6 md:px-8 py-5 text-right border-b border-[#1F2D29]/30">
                  <span
                    className={`px-4 py-1.5 rounded-full text-[9px] font-black tracking-widest uppercase shadow-lg shadow-black/30 ${
                      isLow
